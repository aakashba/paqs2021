...
dataprep = '../qadatasetAstudy/output'
...
class batch_gen(keras.utils.Sequence):
    def __init__(self, seqdata, tt, config, training=True):
        ...
        self.allfids = list(seqdata['c%s' % (tt)].keys())
    ...
    def make_batch(self, batchfids):
        if self.config['batch_maker'] == 'datsonly':
            ...
            return self.divideseqs_ast(batchfids, self.seqdata, self.ansvocabsize, self.tt)
    ...
    def __len__(self):
        ...
        return int(np.ceil(len(list(self.seqdata['c%s' % (self.tt)]))/self.batch_size))
        #else:
    ...
    def divideseqs_ast(self, batchfids, seqdata, ansvocabsize, tt):
        import keras.utils
        conseqs = list()
        ansseqs = list()
        quesseqs = list()
        ...
        for fid in batchfids:
            ...
            wconseq = seqdata['c%s' % (tt)][fid]
            answers = seqdata['a%s' % (tt)][fid]
            questions = seqdata['q%s' % (tt)][fid]
            wconseq = wconseq[:self.config['conlen']]

            for qid in questions:
                if type(answers) != type(astart):
                    wansseq = answers[qid.replace('Q','A')]
                else:
                    wansseq = answers

                wquesseq = questions[qid]
                wconseq = wconseq[:self.config['conlen']]

                if not self.training:
                    fiddat[fid+"-"+qid] = [wconseq,wansseq, wquesseq]
                else:
                    for i in range(0, len(wansseq)):
                        conseqs.append(wconseq)
                        quesseqs.append(wquesseq)
                    # slice up whole comseq into seen sequence and current sequence
                    # [a b c d] => [] [a], [a] [b], [a b] [c], [a b c] [d], ...
                        ansseq = wansseq[0:i]
                        ansout = wansseq[i]
                        ansout = keras.utils.to_categorical(ansout, num_classes=ansvocabsize)

                    # extend length of comseq to expected sequence size
                    # the model will be expecting all input vectors to have the same size
                        for j in range(0, len(wansseq)):
                            try:
                                ansseq[j]
                            except IndexError as ex:
                                ansseq = np.append(ansseq, 0)

                        ansseqs.append(ansseq)
                        ansouts.append(np.asarray(ansout))

        conseqs = np.asarray(conseqs)
        quesseqs = np.asarray(quesseqs)
        ansseqs = np.asarray(ansseqs)
        ansouts = np.asarray(ansouts)

        if not self.training:
            return fiddat
        else:
            return [[conseqs, ansseqs, quesseqs], ansouts]
